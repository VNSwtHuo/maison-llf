...
def train_and_eval(split: dict) -> tuple:
    ...
    metrics = evaluate(model, split["test_ids"], sensor_stats, demo_stats)
    return metrics, epoch


results   = []
out_csv   = DATA_DIR / "search_archive_top_xy_results.csv"
...
for top_x in TOP_X_VALUES:
    for top_y in TOP_Y_VALUES:
        ...
        metrics, epochs = train_and_eval(split)
        mean_item_mae   = (metrics["ohs_item_mae"] + metrics["sis_item_mae"]) / 2.0
        ...
        row = {
            ...
            "ohs_item_mae"  : round(metrics["ohs_item_mae"],   4),
            "ohs_item_rmse" : round(metrics["ohs_item_rmse"],  4),
            "sis_item_mae"  : round(metrics["sis_item_mae"],   4),
            "sis_item_rmse" : round(metrics["sis_item_rmse"],  4),
            "ohs_total_mae" : round(metrics["ohs_total_mae"],  4),
            "ohs_total_rmse": round(metrics["ohs_total_rmse"], 4),
            "sis_total_mae" : round(metrics["sis_total_mae"],  4),
            "sis_total_rmse": round(metrics["sis_total_rmse"], 4),
            "mean_item_mae" : round(mean_item_mae, 4),
            "epochs"        : epochs,
        }
        results.append(row)
        ...
        print(
            ...
            f"OHS_item={metrics['ohs_item_mae']:.3f} "
            f"SIS_item={metrics['sis_item_mae']:.3f} "
            f"mean={mean_item_mae:.3f} "
            f"(ep={epochs})"
        )

        pd.DataFrame(results).sort_values("mean_item_mae").reset_index(drop=True).to_csv(out_csv, index=False)

results_df = pd.DataFrame(results).sort_values("mean_item_mae").reset_index(drop=True)
...
print()
print("=== Top 10 by mean per-item MAE ===")
...
print(
    ...
    f"mean_item_MAE={best['mean_item_mae']:.4f}"
)
